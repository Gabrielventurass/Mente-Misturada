...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($pendente) {
            if ($acao === 'aprovar') {
                $inserir = $pdo->prepare("INSERT INTO admin (nome, email, senha) VALUES (?, ?, ?)");
    ...
    // Listar pendentes
    $lista = $pdo->query("SELECT * FROM admins_pendentes ORDER BY dt_cr DESC")->fetchAll(PDO::FETCH_ASSOC);
